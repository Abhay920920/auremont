      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        ...
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
          <div className="flex-grow space-y-8 animate-fade-in">
            ...
            {safeItems.map((item) => (
              <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center py-6 border-b border-divider/50 group">
                 {/* Product Info */}
                 <div className="col-span-6 flex gap-6 items-center relative">
                    <button 
                      onClick={() => removeItem(item.id)}
                      className="absolute -left-4 md:-left-2 top-0 md:top-auto w-11 h-11 flex items-center justify-center text-mutedText hover:text-error transition-colors"
                      title="Remove Item"
                    >
                    ...
                    <div>
                      <Link href={`/shop/${item.product?.slug}`} className="font-serif text-xl md:text-2xl text-primaryText mb-2 hover:text-luxuryGold transition-colors">
                        {item.product?.name}
                      </Link>
                      <p className="text-secondaryText text-sm mb-1 uppercase tracking-widest">{item.product?.weightGrams || 250}G</p>
                      <p className="text-luxuryGold font-medium mt-2">₹{Number(item.unitPrice).toFixed(2)}</p>
                    </div>
                 </div>

                 {/* Quantity */}
                 <div className="col-span-6 md:col-span-3 flex justify-start md:justify-center items-center pl-36 md:pl-0">
                    <QuantityControl 
                 ...
                 {/* Total */}
                 <div className="col-span-6 md:col-span-3 flex justify-end items-center pr-4 mt-[-40px] md:mt-0">
                    <p className="font-serif text-xl text-primaryText">₹{(item.quantity * Number(item.unitPrice)).toFixed(2)}</p>
                 </div>
              </div>
            ))}

...
export default function ProfileTab({
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-2xl">
      ...
      {profileMsg && (
        <div className={`p-4 border rounded-sm text-sm ${profileMsg.includes('success') ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-error/10 border-error/20 text-error'}`}>
          {profileMsg}
        </div>
      )}

      <form onSubmit={handleUpdateProfile} className="space-y-10 bg-secondaryBg p-8 border border-divider">
